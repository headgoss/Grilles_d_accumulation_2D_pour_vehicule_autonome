...
#include "maps_Grille.h"	// Includes the header of this component
#include <vector>
#include <iostream>
#include <math.h>
#include <stdlib.h>
...
MAPS_BEGIN_INPUTS_DEFINITION(MAPSGrille)
...
MAPS_BEGIN_OUTPUTS_DEFINITION(MAPSGrille)
...
MAPS_BEGIN_PROPERTIES_DEFINITION(MAPSGrille)
//Taille de l'image
	...
	MAPS_PROPERTY("x_start", 0.0, false, true)
...
void MAPSGrille::Core()
{
	...
	for (int i = 0; i < vectorsizeout;i=i+1) {
		data.push_back(ioEltIn->Float64(i));
	...
	*/
	...
	int vitesse_ref = (int)GetIntegerProperty("vit_ref");
	...
	//Distance de freinage pour zone de sécuritéint dfreinage;
	int dfreinage;
	if (vitesse_in < 10)
	...
	*/
	...
	int x_start = (int)GetFloatProperty("x_start");
	MAPSStreamedString sx;
	...
	ioElt->Timestamp() = ioEltIn->Timestamp();
