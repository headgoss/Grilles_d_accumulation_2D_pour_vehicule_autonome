...
#include "maps_ARTEMIPS.h"	// Includes the header of this component
#include <vector>
#include <iostream>
#include <algorithm>
#include <map>
...
MAPS_BEGIN_INPUTS_DEFINITION(MAPSARTEMIPS)
...
MAPS_BEGIN_OUTPUTS_DEFINITION(MAPSARTEMIPS)
...
MAPS_BEGIN_PROPERTIES_DEFINITION(MAPSARTEMIPS)
//Données LIDAR
...
MAPS_BEGIN_ACTIONS_DEFINITION(MAPSARTEMIPS)
//MAPS_ACTION("aName",MAPSGrille::ActionName)
MAPS_END_ACTIONS_DEFINITION

// Use the macros to declare this component (Grille) behaviour
MAPS_COMPONENT_DEFINITION(MAPSARTEMIPS, "MAPSARTEMIPS", "1.0", 128,
...
void MAPSARTEMIPS::Core()
{
	...
	for (int i = 0; i < vectorsizeout; i = i + 1) {
		data.push_back(inputs[0]->Float64(i)*dim);
	...
	//xx.reserve(size);
	//yy.reserve(size);
	vector<pair<float, float>> deux;
	deux.reserve(size);
	int lidar = (int)GetFloatProperty("lidar_voiture");
	auto it_x = x.begin(), it_y = y.begin(), it_z = z.begin();
	while (it_z != z.end()) {
		if ((*it_z >= z_min & *it_z <= z_max & *it_x>=x_start & *it_x<=distance & *it_y>=-y_in &*it_y<=y_in )) {
			
				//xx.push_back(*it_x);
				//yy.push_back(*it_y);
				deux.emplace_back(*it_x, *it_y);
		}
		++it_x;
		++it_y;
		++it_z;
	}
	int taille = deux.size();
	/*
	vector<float> total;
		...
		for (float y = y_in - y_; y >= -y_in; y = y - y_) {
			...
			if (sortir == 0 | tempy != -100) {
				...
				for (int i = 0; i < xx.size();) {
					if (x <= xx[i] & x + x_ >= xx[i] & y <= yy[i] & y + y_ >= yy[i])
					{
						res++;
						xx.erase(xx.begin() + i);
						yy.erase(yy.begin() + i);
				...
				deux.erase(remove_if(deux.begin(), deux.end(), [x, x + x_, y, y + y_](pair<float, float> & point)
				{
				}), deux.end());
				*/
				float x_min = x;
				...
				float y_min = y;
				float y_max = y + y_;
				auto new_data = remove_if(deux.begin(), deux.end(), [&x_min, &x_max, &y_min, &y_max,&res](const auto& point){
					if (point.first >= x_min && point.first <= x_max && point.second >= y_min && point.second <= y_max)
					{
						res++;
						return true;
					}
				...
				deux.erase(new_data, deux.end());

				if (res >= threashold & x < dfreinage)
				{
					if ((x < lidar & y > -0.5 & y < 0.5)) {
						for (int posy = 0; posy < case_size; posy++) {
							for (int posx = col_size_start; posx < col_size_end; posx++) {
								...
								imageData[(posx + posy * taille_ligne + position) * 3 + 2] = (unsigned char)(255);
							}
						}
					}
					else {
						...
						else if (current_index == 1)
						{
							...
							temp = abs(f_angle * (x - darret) / (dfreinage - darret));
						}
